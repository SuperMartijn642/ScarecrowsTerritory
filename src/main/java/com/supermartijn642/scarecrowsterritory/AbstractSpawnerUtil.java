...
public class AbstractSpawnerUtil {
    ...
    private static void tickSpawnerClient(BaseSpawner spawner, Level level, BlockPos pos){
        if(spawner.isNearPlayer(level, pos)){
            ...
            spawner.spin = (spawner.spin + (double)(1000.0F / ((float)spawner.spawnDelay + 200.0F))) % 360.0D;
        }
    }
                ...
                for(int i = 0; i < spawner.spawnCount; ++i){
                    ...
                    if(level.noCollision(optional.get().getAABB(d0, d1, d2))){
                        ...
                        if(entity instanceof Mob){
                            Mob mob = (Mob)entity;
                            if(spawner.nextSpawnData.getCustomSpawnRules().isEmpty() && !mob.checkSpawnRules(level, MobSpawnType.SPAWNER) || !mob.checkSpawnObstruction(level))
                                continue;

                            MobSpawnEvent.FinalizeSpawn event = EventHooks.onFinalizeSpawnSpawner(mob, level, level.getCurrentDifficultyAt(entity.blockPosition()), null, compoundTag, spawner);
                            if(event != null && spawner.nextSpawnData.getEntityToSpawn().size() == 1 && spawner.nextSpawnData.getEntityToSpawn().contains("id", 8))
