                    ...
                    if(level.noCollision(optional.get().getAABB(d0, d1, d2))){
                        ...
                        if(EntitySpawnPlacementRegistry.checkSpawnRules(optional.get(), serverLevel, SpawnReason.SPAWNER, new BlockPos(d0, d1, d2), level.getRandom())){
                            ...
                            if(entity instanceof MobEntity){
                                ...
                                if(!net.minecraftforge.event.ForgeEventFactory.canEntitySpawnSpawner(mobentity, level, (float)entity.x, (float)entity.y, (float)entity.z, spawner)){
                                    continue;
                                }

                                if(getNextSpawnData(spawner).getTag().size() == 1 && getNextSpawnData(spawner).getTag().contains("id", 8)){
                                    if(!net.minecraftforge.event.ForgeEventFactory.doSpecialSpawn(mobentity, level, (float)entity.x, (float)entity.y, (float)entity.z, spawner, SpawnReason.SPAWNER))
                                        ((MobEntity)entity).finalizeSpawn(serverLevel, level.getCurrentDifficultyAt(entity.getCommandSenderBlockPosition()), SpawnReason.SPAWNER, null, null);
                                }
                            }
